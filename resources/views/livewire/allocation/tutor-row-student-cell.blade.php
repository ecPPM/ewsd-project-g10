    ...
    @if($editingTutorId === $user->id)
        <div class="flex flex-col my-4">
            ...
            <input wire:model.live.debounce.500ms="editingStudentName" type="text"
                   ...
                   class="bg-base-100  text-gray-900 text-sm rounded block w-full p-2.5"
                ...
                <div class="relative">
                    <div class="absolute left-0 w-max bg-white shadow-md rounded-md border border-gray-200 mt-1">
                        @foreach ($queryResults as $student)
                            <div wire:key="{{ $student->id }}" wire:click="selectStudent({{ $student->id }})"
                                 class="cursor-pointer px-3 py-1 border-b border-gray-200 hover:bg-base-100 focus:bg-base-100">
                        ...
                        <div class="flex flex-row bg-teal-400 my-1 p-2 justify-between">
                            <div class="flex flex-row ">
                                ...
                                @if ($activeTutor)
                                    ...
                                    @endphp
                                    <div
                                        class="inline-flex items-center self-start ms-2 py-1.5 px-2 rounded text-xs bg-pink-100 font-bold text-pink-500">
                                        <span class="">{{ $initials }}</span>
                                    </div>
                                @endif
                            </div>

                            <button wire:click="deselectStudent({{ $selectedStudent->id }})"
                                    class="text-sm text-red-500 font-semibold rounded hover:text-red-800 flex items-center justify-end w-auto ms-2">
                                <svg class="w-7 h-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512">
                                    <path fill="#fc1d1d"
            ...
            <div class="text-xs mt-2 text-gray-700 flex justify-end items-center">
                @if (sizeof($selectedStudents) > 0)
                    ...
                    <button wire:click="bulkUpdate"
                            class="ms-2 px-4 py-2 bg-teal-500 text-white font-semibold rounded hover:bg-teal-600">Update
                    </button>
                @endif
                <button wire:click="cancelStudentEdit"
                        class="ms-2 px-4 py-2 bg-red-500 text-white font-semibold rounded hover:bg-red-600">Cancel
                </button>
            </div>
        </div>
    @endif
