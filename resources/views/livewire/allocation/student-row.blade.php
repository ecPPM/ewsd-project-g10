    ...
    <td class="py-3 px-6 text-left">
        ...
        @if($editingStudentId === $user->id)
                ...
                <input wire:model.live.debounce.500ms="editingTutorName" type="text"
                    ...
                    @if(sizeof($tutorResults) > 0 && $isSelectingTutor)
                    <div class="relative">
                        <div class="absolute right-0 w-max bg-white shadow-md rounded-md border border-gray-200 mt-1">
                            ...
                            <div wire:key="{{ $tutor->id }}" wire:click="selectTutor({{ $tutor->id }})" class="cursor-pointer px-3 py-1 border-b border-gray-200 hover:bg-gray-100 focus:bg-gray-100">
                                    <div class="flex flex-col ml-3">
                                        <span class="text-base font-medium">{{ $tutor->name }}</span>
        ...
        @else
            ...
            @if($activeTutor)
                <div class="flex flex-col">
                    <span class="text-base font-medium">{{ $activeTutor->name }}</span>
