        ...
        <div class="modal-box flex flex-col w-11/12 max-w-3xl px-5 md:px-10 pt-0 pb-4 md:pb-8">
            <div class="flex flex-col gap-2 relative">
                ...
                @if(count($notes)>0)
                    ...
                    >
                        <div onclick="toggleCollapse()" class="collapse-title cursor-pointer uppercase text-base-content/75 text-base font-semibold ps-0 py-0 flex justify-center items-center rounded-none w-fit">
                            Notes History
                        </div>
                                    ...
                                    @foreach ($notes as $note)
                                        <div class="w-full flex flex-col gap-2">
                                            <div class="flex items-center gap-3">
                                                <span class="text-base text-base-content font-semibold">{{ auth()->user()->id === $note->user->id ? "You" : $note->user->name}}</span>
                                                <div class="w-[6px] h-[6px] rounded-full bg-base-content/50"></div>
                                                <span class="text-xs text-base-content/75">{{ $note->created_at->diffForHumans() }}</span>
                                            </div>
                ...
                <div class="flex gap-3 items-center mt-4 justify-end w-full">
                    ...
                    <button wire:click="addNote" class="btn btn-primary" @if(!$editingNote) disabled @endif>Add Note</button>
                </div>
            </div>
        </div>
</dialog>
